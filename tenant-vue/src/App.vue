...
<template>
  <main class="tenant-app">
    <div v-if="store.connectionError" class="backend-error">
      <strong>无法连接业务服务</strong>
      <p>{{ store.connectionError }}</p>
      <small>请确认后端已启动，并在 tenant-vue/.env.local 配置 VITE_API_BASE_URL、VITE_TENANT_MOBILE 和 VITE_TENANT_PASSWORD。</small>
      <button @click="store.loadRemote">重新连接</button>
    </div>
    
    <RouterView v-else v-slot="{ Component }">
      <KeepAlive>
        <component :is="Component" :key="route.meta.tab || route.path" />
      </KeepAlive>
    </RouterView>
    
    <BottomTabBar v-if="showTabbar && !store.connectionError" />
